...
export const getDistanceDisplay = (km: number, system: UnitSystem, customUnit?: string) => {
...
export const getPaceDisplay = (secondsPerKm: number, system: UnitSystem) => {
  if (secondsPerKm === 0 || !isFinite(secondsPerKm) || secondsPerKm > 3600) return "--'--\"";
...
export const getAltitudeDisplay = (meters: number | null, system: UnitSystem, customUnit?: string) => {
  const defaultUnit = system === 'imperial' ? 'ft' : 'm';
  if (meters === null) return { value: "--", unit: system === 'custom' ? (customUnit || defaultUnit) : defaultUnit };
...
export const getSpeedDisplay = (secondsPerKm: number, system: UnitSystem) => {
  if (secondsPerKm <= 0 || !isFinite(secondsPerKm)) return { value: "0.0", unit: system === 'imperial' ? 'mph' : 'km/h' };
